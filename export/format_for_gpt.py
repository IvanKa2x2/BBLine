...
def to_text(entry):
    ...
    board_str = (
        " ".join(card for card in board[:3] if card)
        + " | "
        + (board[3] or "")
        + " | "
        + (board[4] or "")
    ...
    hero_cards = f"{hero['card1']} {hero['card2']}" + (
        " (сuited)" if hero.get("suited") else " (разномастные)"
    )
    ...
    return (
        f"""Раздача: {hand['hand_id']} ({hand['hero_pos']}), стек {hero['start_stack_bb']}bb → {hero['net_bb']}bb
