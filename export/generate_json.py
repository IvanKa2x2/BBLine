...
def main():
    ...
    for r in fetchall(
        "SELECT hand_id, street, seat, action, amount_bb FROM actions ORDER BY action_id"
    ):
    ...
    with open(OUT_FILE, "w", encoding="utf-8") as f_out:
        for hand_id, base in hand_base.items():
            hero_info = next(
                (p for p in players_by_hand[hand_id] if "Hero" in p["player_id"]), None
            )
            if not hero_info:
                continue
            ...
            opps = [
                ...
                for p in players_by_hand[hand_id]
                if "Hero" not in p["player_id"]
            ]
